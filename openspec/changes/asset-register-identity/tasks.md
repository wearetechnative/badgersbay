...
- [ ] 1.1 Define `assets.csv` columns: `asset_id, serial, owner, model, class,
      status, owner_since, valid_from, valid_to`
- [ ] 1.2 Loader with fail-fast validation: duplicate active serial, unknown
      class, unknown status, empty serial, overlapping validity for one serial
- [ ] 1.5 Allow several rows per `asset_id` with distinct serials and validity
      windows; `asset_id` is the identity, serial the lookup key
- [ ] 1.6 `status: retired` excludes an entry from the round denominator while
      keeping it visible and keeping its history
- [ ] 1.3 `config.yaml`: `compliance.asset_register: ./assets.csv`
- [ ] 1.4 Startup logs the register size; absent register disables the round and
...
- [ ] 4.1 Rewrite `get_audit_period()` to map backward to the most recent audit
      month at or before the date
- [ ] 4.2 Classify each submission `on_time` or `late` relative to its round
- [ ] 4.5 `grace_weeks` config value defines both the on_time/late boundary and
      the scan window used for scope membership
- [ ] 4.3 Remove audit period from all filesystem paths
- [ ] 4.4 Doctests covering 2026-09-15, 2026-10-02, 2026-04-10, 2026-02-10
...
- [ ] 6.3 Read `reports/2026-03/` as archive records without serials
...
- [ ] 7.4 Unmatched submissions surfaced as a warning block
